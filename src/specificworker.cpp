...
bool SpecificWorker::buscarCaja()
{
	tag t;
	...
	if(tagslocal0.getIdCaja(t,vectorCajas))
	{
		numMarca=t.id;
		qDebug()<<"CAMBIO DE MARCA "<< numMarca;
		primera=false;
		S=STATE::R;
	}
	else{
		if(reloj.elapsed() > 7000){
			qDebug()<<"FINISH NO HAY MAS CAJAS";
			primera=false;
...
void SpecificWorker::soltarCaja()
{
	...
	enmarca=false;
	marencontrada=false;
	//PARA NO GUARDAR LA MARCA ANTERIOR UNA VEZ LA SOLTAMOS.
	numMarca=9999;
	S = STATE::IR;
}
